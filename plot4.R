...
plot4<-function(){
    ...
    data5<-cbind(pasted,data1[,3])                         # Create matrix DateTime and kW
    data5<-na.omit(data5)                                  # Remove incomplete entries
    
    data6<-cbind(pasted,data1[,5])                         # Create matrix DateTime and V
    data6<-na.omit(data6)                                  # Remove incomplete entries
    
    data7<-cbind(pasted,data1[,4])                         # Create matrix DateTime and VAR
    data7<-na.omit(data7)                                  # Remove incomplete entries
    
    #-------------------------------------------------------------------------------#
    #  Create plot.                                                                 #
    #-------------------------------------------------------------------------------#
    
    png(file="plot4.png", width = 480, height = 480)       # Create png to specification  
    
    par(mfrow=c(2,2))                                      # Print 4 plots to the page
    
    
    # Plot 1: upper lefthand corner
    #----------------------------------------------------
    plot(as.numeric(data5[,2]),type="l",xaxt="n",xlab="",  # Plot kW as a line plot against
         ylab="Global Active Power");                      # a generic x axis. No label on
    ...
    # Plot 2: upper righthand corner
    #-----------------------------------------------------
    plot(as.numeric(data6[,2]),type="l",xaxt="n",          # Plot V as a line plot against
         xlab="datetime",ylab="Voltage");                  # a generic x axis. Appropriate
                                                           # names on y axis and x axis.
    axis(1, at = c(0,1440,2880), labels = days,            # Create x axis labels for days of week
         tick = TRUE)                                      # as appropriate
    #-----------------------------------------------------
    ...
    legend("topright",                                     # Create legend in top right corner
           legend=c("Sub_metering_1","Sub_metering_2",     # with specified formatting
                    "Sub_metering_3"), pch="___",          # No boarder
           col=c("black","red","blue"),bty="n")
    
    par(new=FALSE)                                         # Move on to next plot
    #----------------------------------------------------
    
    
    # Plot 4: lower righthand corner
    #----------------------------------------------------
    plot(as.numeric(data7[,2]),type="l",xaxt="n",          # Plot VAR as a line plot against
         xlab="datetime",ylab="Global_reactive_power");    # a generic x axis. No label on
                                                           # x axis. Appropriate name on y axis
    axis(1, at = c(0,1440,2880), labels = days,            # Create x axis labels for days of week
         tick = TRUE)                                      # as appropriate
    #----------------------------------------------------
